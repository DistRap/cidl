...
module Cidl.Parse (parseDecls, ppDecl) where
...
-- structures Cidl uses elsewhere. That way, we can separate our
...
toEnv decls = do
  ...
  where (typDs, ifcDs) = partition isTypeDecl decls
        ...
        toMethod (n, AttrDecl addr perm t) = do
          t' <- runReaderT (getType t) []
          return (n, AttrMethod addr perm t')
...
parseDecls = return . pack >=> decode cidlSpec >=> toEnv

cidlSpec :: SExprParser HaskLikeAtom Decl
cidlSpec
  = withHaskellComments
  $ setCarrier tDecl
...
-- some parsing of cidl-specific types
tBits :: Parse Bits
...
tInterfaceMethod e =
  ...
  where go ["attr",   a, p, t] = AttrDecl   <$> tAddr a <*> tPermission p <*> tType t
...
ppMethod (AttrDecl addr perm name) = L [ "attr", int addr, ppPerm perm, ident name ]
  where ppPerm Read      = "r"
        ppPerm Write     = "w"
        ppPerm ReadWrite = "rw"
